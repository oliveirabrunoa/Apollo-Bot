...
async def get_lists(ctx):
  ...
  if not user_listas or len(user_listas) == 0:
    embed = embed_settings.info(
      "``Não existem listas criadas para este usuário``", "")
...
async def add_list(ctx):
  ...
  if 'note' in ctx.message.content:
    await ctx.send(
      "Digite o #ID da lista para qual deseja adicionar uma anotação:")
    ...
    result = queries.create_or_update_note(int(list_id), note_content,
                                           ctx.author.id)
    if result is True:
      embed = embed_settings.successful(f"```Nota adicionada com sucesso!```")
      await ctx.send(embed=embed)
    if result is False:
      embed = embed_settings.failure(
        f"``Não foi possível adicionar esta nota!``",
  ...
  if 'lista' in ctx.message.content:
    ...
    embed = embed_settings.successful(
      f"```Lista {message.content} foi CRIADA com sucesso!```")
    await ctx.send(embed=embed)
...
async def delete_list(ctx):
  ...
  if 'lista' in ctx.message.content:
    await ctx.send(
      "Digite o #ID da lista que deseja deletar. \n **ATENÇÃO:** esta ação não poderá ser desfeita!"
    )
    user_listas = queries.get_list_user(ctx.author.id)
    ...
    if result is True:
      await ctx.send(embed=embed_settings.successful(
        f"```Lista {message.content} foi deletada com sucesso!```"))
    if result is False:
      await ctx.send(embed=embed_settings.successful(
        f"```Não foi possível deletar a lista informada!```",
        "Algumas causas possíveis: \n - Não existe lista com o #ID informado \n - A lista informada não pertence ao usuário e não pode ser deletada por outro(s)"
      ))

  if 'tarefa' in ctx.message.content:
    await ctx.send(
      "Digite o #ID da lista que contém a tarefa que deseja deletar. \n **ATENÇÃO:** esta ação não poderá ser desfeita!"
    )
    user_listas = queries.get_list_user(ctx.author.id)
    #await ctx.send(user_listas)
    ...
    message = await bot.wait_for(
      'message', check=lambda message: message.author == ctx.author)
    result = queries.delete_task_list_user(list_id, ctx.author.id,
                                           message.content)
    if result is True:
      await ctx.send(
        embed=embed_settings.successful(f"```Tarefa deletada com sucesso!```"))
    if result is False:
      await ctx.send(embed=embed_settings.failure(
...
async def get_tasks_by_list_id(ctx):
  ...
  if not tasks_list or len(tasks_list) == 0:
    embed = embed_settings.failure(
      "``Não foi possível consultar as Tarefas para o #ID informado!``",
...
async def update_tasks(ctx):
  ...
  if not update_task or len(update_task) <= 0:
    ...
    await ctx.send(
      'Digite o #ID do Status para o qual deseja atualizar a tarefa selecionada:'
    )
    await ctx.send(embed=embed_settings.get_all_status_task())
    message = await bot.wait_for(
      'message', check=lambda message: message.author == ctx.author)
    update_task_status = queries.update_task(update_task.get('task_id'),
                                             message.content, ctx.author.id)
    if update_task_status is False:
      await ctx.send(embed=embed_settings.failure(
        "Não foi possível atualizar a tarefa informada",
        "Verifique o status informado e tente novamente."))
    else:
      await ctx.send(embed=embed_settings.successful(
...
async def all_tasks_by_user(ctx):
  ...
  if not tasks_list or len(tasks_list) == 0:
    embed = embed_settings.failure(
      f"``Não foi possível consultar suas tarefas!``",
      'Verifique o ID e tente novamente')
    await ctx.send(embed=embed)
  else:
